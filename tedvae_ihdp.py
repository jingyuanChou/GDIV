...
from datasets import IHDP, GraphDVAE_Data
import numpy as np
...
def main(args, dataset_path, reptition=1, path="./IHDP/"):
    ...
    # train, test, contfeats, binfeats = IHDP(dataset_path=dataset_path, path=path, reps=reptition, cuda=False)
    train, test, contfeats, binfeats = GraphDVAE_Data(dataset_path=dataset_path, name=args.name, path=path, reps=reptition, cuda=False)
    ...
    pehe = np.sqrt(
        np.mean((true_ite_test.squeeze() - est_ite.cpu().numpy()) * (true_ite_test.squeeze() - est_ite.cpu().numpy())))
    pehe_train = np.sqrt(np.mean((true_ite_train.squeeze() - est_ite_train.cpu().numpy()) * (
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature-dim", default=2246, type=int)
    ...
    parser.add_argument('--name', type=str, default='BlogCatalogGraphDVAE')
    ...
    for i in range(10):
        ...
        tedvae_pehe[i, 0], tedvae_pehe_train[i, 0] = main(args, args.dataset_path, i + 1, path)
