...
def IHDP(dataset_path, path="./IHDP", reps=1, cuda=True):
    path_data = path
    replications = reps
    # which features are binary
...
def GraphDVAE_Data(dataset_path, name, path="./IHDP", reps=1, cuda=True):
    path_data = path
    replications = reps

    Graph_data = sio.loadmat(dataset_path + name + '/' + name + str(replications) + '.mat')
    A = Graph_data['Network']
    X = Graph_data['X_100']
    n_feature = X.shape[1]
    # which features are binary
    binfeats = []
    # which features are continuous
    contfeats = [i for i in range(n_feature) if i not in binfeats]

    # csr matrix
    Y = Graph_data['Y_actual']
    Y1 = Graph_data['Y1']
    Y0 = Graph_data['Y0']
    T = Graph_data['T']
    ...
    n = t.size(0)
    idx = np.random.permutation(t.size(0))

    split_train = int(0.8 * n)

    idx_train, idx_test = idx[:split_train], idx[split_train:]

    x_train = x[idx_train]
    t_train = t[idx_train]
    y_train = y[idx_train]
    ite_train = true_ite.transpose()[idx_train]

    train = (x_train, t_train, y_train), ite_train

    x_test = x[idx_test]
    t_test = t[idx_test]
    y_test = y[idx_test]
    ite_test = true_ite.transpose()[idx_test]

    test = (x_test, t_test, y_test), ite_test
    return train, test, contfeats, binfeats
